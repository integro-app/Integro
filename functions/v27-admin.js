...
function criarAdministracaoV27({ admin, functions, db }) {
  ...
  async function userProfileByEmail(email) {
    const normalized = lower(email);
    if (!normalized || !normalized.includes("@")) return null;
    let query = await db.collection("usuarios").where("emailNormalizado", "==", normalized).limit(2).get();
    if (query.empty) query = await db.collection("usuarios").where("email", "==", normalized).limit(2).get();
    if (query.empty) return null;
    const doc = query.docs[0];
    return { id: doc.id, ...doc.data() };
  }
  ...
  async function iniciarSessao(data, context) {
    ...
    if (Number(actor.tentativasLoginFalhas || 0) > 0) {
      await db.collection("usuarios").doc(actor.id || uid).set({ tentativasLoginFalhas:0, ultimaAutenticacaoValidaEm:serverTimestamp() }, { merge:true });
    }
  ...
  async function registrarFalhaLogin(data) {
    const email = lower(data?.email);
    if (!email || !email.includes("@")) return { ok:true, bloqueado:false };
    const target = await userProfileByEmail(email);
    // Resposta deliberadamente genérica para não revelar se o e-mail existe.
    if (!target || !target.authUid || !tenant(target)) return { ok:true, bloqueado:false };
    const config = await configForTenant(tenant(target));
    const max = Math.max(1, Math.min(Number(config?.seguranca?.maxTentativasLogin || 5), 20));
    const ref = db.collection("usuarios").doc(target.id);
    let attempts = 0; let blocked = false;
    await db.runTransaction(async tx => {
      const snap = await tx.get(ref); if (!snap.exists) return;
      const current = snap.data() || {};
      if (["INATIVO","SUSPENSO"].includes(upper(current.status))) return;
      attempts = Number(current.tentativasLoginFalhas || 0) + 1;
      blocked = attempts >= max;
      tx.set(ref, {
        tentativasLoginFalhas:attempts, ultimaFalhaLoginEm:serverTimestamp(),
        ...(blocked ? { status:"BLOQUEADO", acessoLiberado:false, bloqueioMotivo:"TENTATIVAS_LOGIN", bloqueadoEm:serverTimestamp() } : {})
      }, { merge:true });
    });
    if (blocked) {
      await admin.auth().updateUser(text(target.authUid), { disabled:true }).catch(() => {});
      await admin.auth().revokeRefreshTokens(text(target.authUid)).catch(() => {});
      await db.collection("sessoes_usuarios").doc(text(target.authUid)).set({ encerrada:true, expiresAtMs:0, motivoEncerramento:"TENTATIVAS_LOGIN", atualizadoEm:serverTimestamp() }, { merge:true });
      await notifySecurity({ ...target, authUid:text(target.authUid) }, null, "USUARIO_BLOQUEADO", `Seu acesso foi bloqueado após ${max} tentativas inválidas de login. Solicite o desbloqueio a um superior autorizado.`);
    }
    return { ok:true, bloqueado:blocked, tentativasRestantes:Math.max(0,max-attempts) };
  }
  ...
  return { iniciarSessao, validarSessao, encerrarSessao, registrarFalhaLogin, resetPassword, desbloquearUsuario, bloquearUsuario, invalidarSessoes, gerarTokenOperacao };
}
