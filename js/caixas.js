...
async function consultarCaixasColecao(colecao, tenantId, limite, forcar = false, cacheMs = 20000) {
  ...
  if (runtime?.consultarTenant) {
    return runtime.consultarTenant({
      ...
      cacheMs,
      forcar,
      chave: `caixas-base:${colecao}:${tenantId}:${limite}`
    });
  }
  const snap = await db.collection(colecao)
    ...
    .limit(limite)
    .get();
  return snap.docs.map(doc => ({ id: doc.id, ...doc.data() })).filter(item => item.excluido !== true);
}

async function carregarDadosIniciaisSupervisaoCaixas(forcar = false, incluirEstrutura = telaCaixasAtiva()) {
  ...
  try {
    const caixasPromise = consultarCaixasColecao(
      CONFIG.COLECOES.CAIXAS,
      tenantId,
      CONFIG.LIMITS?.CAIXAS || 500,
      forcar,
      10000
    );

    if (!incluirEstrutura) {
      caixasCaixaTempoReal = await caixasPromise;
      State.setCaixas?.(caixasCaixaTempoReal);
      return;
    }

    const [equipes, usuarios, caixas] = await Promise.all([
      consultarCaixasColecao(CONFIG.COLECOES.EQUIPES, tenantId, CONFIG.LIMITS?.EQUIPES || 300, forcar, 60000),
      consultarCaixasColecao(CONFIG.COLECOES.USUARIOS, tenantId, CONFIG.LIMITS?.USUARIOS || 500, forcar, 30000),
      caixasPromise
    ]);

    equipesCaixaTempoReal = equipes;
    vendedoresCaixaTempoReal = usuarios;
    caixasCaixaTempoReal = caixas;

    State.setEquipes?.(equipesCaixaTempoReal);
    State.setUsuarios?.(vendedoresCaixaTempoReal);
    State.setCaixas?.(caixasCaixaTempoReal);
...
function iniciarCaixasTempoReal() {
  const tenantId = getTenantIdSeguroCaixas();
  if (!tenantId || !telaCaixasAtiva() || caixasRealtimeIniciado) return;

  pararCaixasTempoReal();
  caixasRealtimeIniciado = true;
  carregarDadosIniciaisSupervisaoCaixas(false, true);

  const runtime = window.IntegroDataRuntime;
  if (runtime?.ouvir) {
    unsubscribeCaixasTempoReal = runtime.ouvir({
      chave: `tela-caixas:${tenantId}:caixas`,
      escopo: "tela:caixas",
      db,
      colecao: CONFIG.COLECOES.CAIXAS,
      tenantId,
      limite: CONFIG.LIMITS?.CAIXAS || 500,
      aoAtualizar: dados => {
        caixasCaixaTempoReal = dados;
        State.setCaixas?.(dados);
        limparSelecaoInvalidaEquipesCaixa();
        if (equipeDetalheAtualId) renderDetalhesEquipeSupervisao(equipeDetalheAtualId);
        else renderCaixas();
      },
      aoErro: erro => console.error("[SUPERVISÃO CAIXAS] erro realtime caixas:", erro)
    });
    return;
  }

  unsubscribeCaixasTempoReal = db.collection(CONFIG.COLECOES.CAIXAS)
    .where("clientePlataformaId", "==", tenantId)
...
function pararCaixasTempoReal() {
  ...
  window.IntegroDataRuntime?.pararEscopo?.("tela:caixas");
...
async function carregarCaixas(forcar = false) {
  // A carga inicial do painel precisa apenas do saldo dos caixas. Equipes e
  // usuários são buscados quando a tela Caixas é realmente aberta.
  await carregarDadosIniciaisSupervisaoCaixas(forcar, telaCaixasAtiva());
}
...
function controladorCaixasExternoAtivo() {
  return !window.renderCaixasPremium || window.renderCaixasPremium === renderCaixasPremium;
}

function renderCaixasPremium(forcar = false) {
  if (!controladorCaixasExternoAtivo()) return Promise.resolve();
  if (!podeInicializarSupervisaoCaixas()) return Promise.resolve();
  prepararTelaCaixas();
  if (!telaCaixasAtiva()) return carregarDadosIniciaisSupervisaoCaixas(forcar, false);

  return carregarDadosIniciaisSupervisaoCaixas(forcar, true).then(() => {
    iniciarCaixasTempoReal();
    renderCaixas();
  });
}
...
document.addEventListener("integro-tela-alterada", evento => {
  if (!controladorCaixasExternoAtivo()) return pararCaixasTempoReal();
  if (evento.detail?.tela === "caixas") {
    renderCaixasPremium(false);
  } else {
    pararCaixasTempoReal();
  }
});
...
document.addEventListener("usuario-validado", () => {
  prepararTelaCaixas();
  if (controladorCaixasExternoAtivo() && telaCaixasAtiva() && podeInicializarSupervisaoCaixas()) renderCaixasPremium(false);
});

window.addEventListener("beforeunload", pararCaixasTempoReal);
