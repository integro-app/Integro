(function (global) {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .integro-nc-list{flex:1;overflow:auto;padding:10px 16px 24px}.integro-nc-item{display:grid;grid-template-columns:26px 1fr auto;gap:10px;padding:14px 10px;border-bottom:1px solid #edf2f7;cursor:pointer}.integro-nc-item.unread{background:#f8fbff}.integro-nc-item input{margin-top:4px}.integro-nc-item h3{margin:0 0 4px;color:#0f172a;font-size:14px}.integro-nc-item p{margin:0;color:#52657f;font-size:13px;line-height:1.35}.integro-nc-meta{margin-top:7px;color:#94a3b8;font-size:11px;font-weight:700}.integro-nc-dot{width:8px;height:8px;border-radius:50%;background:#ff8a00;margin-top:5px}.integro-nc-item:not(.unread) .integro-nc-dot{background:#cbd5e1}.integro-nc-empty{margin:18px;border:1px dashed #cbd5e1;border-radius:14px;padding:28px;text-align:center;color:#64748b;font-weight:700}
  ...
  function ensureDom() {
    ...
    root.querySelector("[data-select-all]").onchange = event => {
      const snapshot = filteredItems();
      selected = event.target.checked ? new Set(snapshot.map(item => item.id)) : new Set();
      render();
    };
    root.querySelectorAll("[data-bulk]").forEach(button => button.onclick = () => bulk(button.dataset.bulk));
  }

  function snapshot() { return global.IntegroNotificationStore?.snapshot?.() || { items: [], unreadCount: 0 }; }
  function filteredItems() {
    return snapshot().items.filter(item => filter === "TODAS" || (filter === "NAO_LIDAS" ? item.lida !== true : item.lida === true));
  }
  ...
  function renderBadges(count) {
    ...
    ["badgeNotificacoes","notificationCount","contadorNotificacoes","badgeNotificacoesMenu"].forEach(id => {
      const el = document.getElementById(id); if (!el) return;
      el.textContent = count > 99 ? "99+" : String(count); el.hidden = count === 0;
    });
  }
  ...
  function render() {
    ...
    list.innerHTML = items.length ? items.map(item => `<article class="integro-nc-item ${item.lida ? "" : "unread"}" data-id="${esc(item.id)}"><input type="checkbox" data-select="${esc(item.id)}" ${selected.has(item.id) ? "checked" : ""}><div><h3>${esc(item.titulo || item.tipo || "Notificação")}</h3><p>${esc(item.mensagem || "")}</p><div class="integro-nc-meta">${esc(item.categoria || item.origemTipo || "Sistema")} ${timeLabel(item) ? `• ${esc(timeLabel(item))}` : ""}</div></div><span class="integro-nc-dot" aria-hidden="true"></span></article>`).join("") : '<div class="integro-nc-empty">Nenhuma notificação nesta visão.</div>';
    list.querySelectorAll("[data-select]").forEach(input => input.onclick = event => {
  ...
  function open() { ensureDom(); document.getElementById("integroNotificationCenter").hidden = false; render(); }
  function close() { const root = document.getElementById("integroNotificationCenter"); if (root) root.hidden = true; }
  ...
  function install() {
    ...
    global.abrirGavetaNotificacoesVendedor = open;
    global.abrirGavetaNotificacoesMaster = open;
    global.abrirNotificacoes = open;
    global.renderizarNotificacoes = render;
    if (!global.__integroAbrirComunicacaoNotificacoesV25) {
      const anterior = global.abrirComunicacaoMasterLocal;
      global.abrirComunicacaoMasterLocal = function(modulo) {
        if (String(modulo || "").toLowerCase() === "notificacoes") { open(); return true; }
        return typeof anterior === "function" ? anterior.apply(this, arguments) : undefined;
      };
      global.__integroAbrirComunicacaoNotificacoesV25 = true;
    }
    global.IntegroNotifications?.install?.();
  }

  global.IntegroNotificationCenter = Object.freeze({ open, close, render, install });
