(function (global) {
  ...
  function db() {
    return global.db || global.firebase?.firestore?.() || null;
  }

  function user() {
    return global.State?.getUsuario?.() || global.obterUsuarioAtual?.() || global.usuarioLogado || global.usuarioAtual || null;
  }

  function authUid() {
    return text(global.firebase?.auth?.()?.currentUser?.uid || user()?.authUid || user()?.uid);
  }
  ...
  function sortNotifications(a, b) {
    return timestampValue(b.criadoEm || b.atualizadoEm || b.criadoEmTexto) - timestampValue(a.criadoEm || a.atualizadoEm || a.criadoEmTexto);
  }
  ...
  function storeSet(list) {
    const filtered = (list || []).map(normalize).filter(item => belongs(item)).filter(item => !item.excluida).sort(sortNotifications);
    global.IntegroNotificationStore?.set?.(filtered);
    global.notificacoesLayout = filtered;
  ...
  async function list(options = {}) {
    ...
    const snapshot = await database.collection("notificacoes")
      .where("destinatarioAuthUid", "==", uid)
      .limit(limit)
      .get();
  ...
  async function updateOwn(id, patch) {
    ...
    await database.collection("notificacoes").doc(id).set({
      ...patch,
  ...
  async function markRead(id) {
    return updateOwn(id, { lida: true, status: "LIDA", lidaEm: serverTimestamp(), lidaPor: authUid() });
  }

  async function markUnread(id) {
    return updateOwn(id, { lida: false, status: "PENDENTE", lidaEm: null, lidaPor: "" });
  }
  ...
  function safeId(value) {
    return text(value).replace(/[^a-zA-Z0-9_-]/g, "_").slice(0, 220);
  }

  function routeFor(input = {}) {
    if (input.rota) return input.rota;
    const type = upper(input.tipo || input.origemTipo);
    if (type.includes("LEAD") || input.indicacaoId || input.clienteOperacionalId) {
      return { tela: "clientes", aba: "leads", entidadeId: text(input.clienteOperacionalId), acao: "ABRIR_DRAWER" };
    }
    if (type.includes("MOVIMENT") || input.movimentacaoId || input.solicitacaoId) {
      return { tela: "movimentacoes", entidadeId: text(input.movimentacaoId || input.solicitacaoId), acao: "ABRIR_DETALHE" };
    }
  ...
  async function emit(input = {}, options = {}) {
    ...
    const payload = {
      ...
      tipo: upper(input.tipo || "SISTEMA"),
      categoria: upper(input.categoria || input.origemTipo || "SISTEMA"),
      prioridade: upper(input.prioridade || "NORMAL"),
      titulo: text(input.titulo || "Notificação"),
      mensagem: text(input.mensagem),
      origemModulo: upper(input.origemModulo || input.origemTela),
      origemEvento: upper(input.origemEvento || input.tipo),
      origemTipo: upper(input.origemTipo),
      origemId: text(input.origemId || eventId),
      entidadeTipo: upper(input.entidadeTipo || (input.indicacaoId ? "LEAD" : input.movimentacaoId ? "MOVIMENTACAO" : "")),
      entidadeId: text(input.entidadeId || input.clienteOperacionalId || input.movimentacaoId || input.solicitacaoId),
      indicacaoId: text(input.indicacaoId),
      clienteOperacionalId: text(input.clienteOperacionalId),
      solicitacaoId: text(input.solicitacaoId),
      movimentacaoId: text(input.movimentacaoId),
      caixaId: text(input.caixaId),
      rota: routeFor(input),
      idempotencyKey,
      eventoId: eventId,
      lida: false,
      ...
      status: "PENDENTE",
      ativo: true,
      criadoPorAuthUid: text(input.criadoPorAuthUid || authUid()),
  ...
  const api = Object.freeze({ list, subscribe, unsubscribe, markRead, markUnread, remove, emit, open, install, belongs, normalize, get authUid() { return authUid(); }, get tenantId() { return tenantId(); } });
