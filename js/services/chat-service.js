(function (global) {
  ...
  const HISTORICO_TEMPORARIO_HORAS = 24;
  ...
  function podeCriarGrupo(usuario = usuarioAtual()) {
    if (!podeEnviar(usuario)) return false;
    const autorizados = ["master_local", "gerente", "supervisor", "financeiro"];
    return perfisUsuario(usuario).some(perfil => autorizados.includes(perfil));
  }
  ...
  function conversaGrupoId(tenant) {
    const aleatorio = Math.random().toString(36).slice(2, 10);
    return `grupo_${normalizarId(tenant)}_${Date.now().toString(36)}_${aleatorio}`;
  }
  ...
  function timestampFuturo(horas = HISTORICO_TEMPORARIO_HORAS) {
    const data = new Date(Date.now() + Math.max(1, Number(horas || HISTORICO_TEMPORARIO_HORAS)) * 60 * 60 * 1000);
    return global.firebase?.firestore?.Timestamp?.fromDate?.(data) || data;
  }
  ...
  async function listarUsuariosDisponiveis() {
    ...
    if (!tenant) return [];
    const snap = await db().collection("usuarios").where("clientePlataformaId", "==", tenant).limit(180).get();
    const usuario = usuarioAtual();
  ...
  async function listarConversas() {
    ...
    const snap = await db().collection(COLECAO).where("clientePlataformaId", "==", tenant)
      .where("participantesIds", "array-contains", remetenteId).limit(120).get();
  ...
  function assinarConversas(callback, onErro) {
    ...
    return db().collection(COLECAO).where("clientePlataformaId", "==", tenant)
      .where("participantesIds", "array-contains", remetenteId).limit(120)
  ...
  function assinarMensagens(conversaId, callback, onErro) {
    ...
    return db().collection(COLECAO).doc(conversaId).collection(SUBCOLECAO)
      .orderBy("criadoEm", "asc").limit(180)
      .onSnapshot(snap => {
        const agora = Date.now();
        const mensagens = snap.docs.map(normalizarDoc).filter(item => !item.expiraEm || millis(item.expiraEm) > agora);
        callback(mensagens);
      }, erro => onErro?.(erro));
  }
  ...
  async function criarOuObterConversaDireta(destinatario) {
    ...
    if (!snap.exists) {
      try {
        await ref.set({
          clientePlataformaId: tenant, tipo: "DIRETA", subtipo: "DIRETA", historicoModo: "COMUM",
  ...
  async function criarOuObterConversaEquipe(equipeId, participantesIds = []) {
    ...
    if (!snap.exists) {
      await ref.set({
        clientePlataformaId: tenant, tipo: "EQUIPE", subtipo: "EQUIPE", equipeId: equipe, participantesIds: ids,
        participantesNomes: { [remetenteId]: nomeUsuario(usuario) }, historicoModo: "COMUM",
        ultimaMensagem: "", ultimaMensagemEm: "", naoLidasPorUsuario: {}, criadoEm: timestamp(), atualizadoEm: timestamp()
      });
    }
    return { id, ...(snap.exists ? snap.data() : {}) };
  ...
  async function criarGrupo({ nome, participantesIds = [], historicoModo = "COMUM" } = {}) {
    const { usuario, tenant, remetenteId } = validarContextoEnvio();
    if (!podeCriarGrupo(usuario)) throw new Error("Seu perfil nao possui permissao para criar grupos.");
    const grupoNome = String(nome || "").trim().slice(0, 80);
    if (grupoNome.length < 2) throw new Error("Informe um nome para o grupo.");
    const idsSelecionados = [...new Set(participantesIds.map(String).filter(Boolean).filter(id => id !== remetenteId))];
    if (!idsSelecionados.length) throw new Error("Selecione pelo menos um participante.");

    const perfis = await Promise.all(idsSelecionados.map(async id => {
      const snap = await db().collection("usuarios").doc(id).get();
      const perfil = snap.exists ? normalizarDoc(snap) : null;
      if (!perfil || perfil.authUid !== id || perfil.clientePlataformaId !== tenant || !estaAtivo(perfil)) {
        throw new Error("Um dos participantes selecionados nao esta ativo ou vinculado a esta empresa.");
      }
      return perfil;
    }));

    const ids = [remetenteId, ...idsSelecionados].sort();
    const nomes = { [remetenteId]: nomeUsuario(usuario) };
    perfis.forEach(perfil => { nomes[perfil.authUid] = nomeUsuario(perfil); });
    const modo = String(historicoModo || "COMUM").toUpperCase() === "TEMPORARIO" ? "TEMPORARIO" : "COMUM";
    const id = conversaGrupoId(tenant);
    const ref = db().collection(COLECAO).doc(id);
    await ref.set({
      clientePlataformaId: tenant,
      tipo: "EQUIPE",
      ...
      grupoNome,
      historicoModo: modo,
      historicoHoras: modo === "TEMPORARIO" ? HISTORICO_TEMPORARIO_HORAS : 0,
    ...
    return { id, grupoNome, tipo: "EQUIPE", subtipo: "GRUPO", historicoModo: modo, participantesIds: ids, participantesNomes: nomes };
  }
  ...
  async function registrarMensagem(conversaId, dadosMensagem, previa) {
    ...
    await db().runTransaction(async tx => {
      ...
      const temporaria = String(conversa.historicoModo || "COMUM").toUpperCase() === "TEMPORARIO";
      const payload = {
        ...dadosMensagem,
        criadoEm,
        dataOperacional: dataOperacionalSaoPaulo(),
        status: "ENVIADA",
        historicoModo: temporaria ? "TEMPORARIO" : "COMUM",
        ...(temporaria ? { expiraEm: timestampFuturo(conversa.historicoHoras || HISTORICO_TEMPORARIO_HORAS) } : {})
      };
      tx.set(mensagemRef, payload);
  ...
  global.IntegroChatService = {
    ...
    criarOuObterConversaDireta, criarOuObterConversaEquipe, criarGrupo, enviarMensagem, enviarImagem, marcarComoLida,
    conversaDiretaId, conversaEquipeId, conversaGrupoId, dataOperacionalSaoPaulo, usuarioAtual, usuarioId, tenantAtual,
    estaAtivo, podeEnviar, podeCriarGrupo, somenteLeitura, iniciarPresenca, assinarPresencas, statusPresencaEfetivo,
    publicarPresenca, HISTORICO_TEMPORARIO_HORAS
  };
})(window);
