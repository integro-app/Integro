(function (global) {
  ...
  function accountPayload(input = {}) {
    ...
    return {
      ...
      responsavelAuthUid: text(input.responsavelAuthUid), responsavelNome: text(input.responsavelNome),
  ...
  async function criarConta(input = {}, options = {}) {
    ...
    return { id: ref.id, ...data, statusCalculado: normalizeStatus(data) };
  }
  ...
  async function atualizarConta(id, patch = {}) {
    ...
    await audit("EDITAR_CONTA", "CONTA", id, before, after); return { id, ...after, statusCalculado: normalizeStatus(after) };
  }
  ...
  async function salvarCategoria(input, id = "") { return saveCatalog(COLLECTIONS.categorias, "CATEGORIA", input, id); }
  ...
  async function criarRecorrencia(input = {}) {
    ...
    const data={
      ...
      valorCentavos:Math.abs(cents(input.valorCentavos ?? input.valor)), unidade:text(input.unidade || "MES").toUpperCase(), intervalo:Math.max(1,Number(input.intervalo||1)), dataInicio:start, dataFim:text(input.dataFim), limiteOcorrencias:Math.max(0,Number(input.limiteOcorrencias||0)), ocorrenciasGeradas:0, proximaGeracao:start, ativo:true,
  ...
  async function gerarOcorrenciasRecorrencia(recorrenciaId, options = {}) {
    ...
    while(due && due<=horizon && created.length<max) {
      ...
      const occurrence=generated+1; const accountId=`rec_${recorrenciaId}_${due.replace(/-/g,"")}`;
      const account=await criarConta({ ...rec, vencimento:due, recorrenciaId, recorrente:true }, { id:accountId, auditExtra:{recorrenciaId,ocorrencia:occurrence} }); created.push(account); generated++; due=recurrenceNext(due,rec.unidade,rec.intervalo);
    }
  ...
  function resumoRelatorios(accounts = [], payments = []) {
    ...
    return {totalAbertoCentavos:sum(active),totalPagoCentavos:sum(payments,"valorEfetivoCentavos"),porCategoria:group(accounts,"categoriaNome","valorCentavos"),porFornecedor:group(accounts,"fornecedorNome","valorCentavos"),porEmpresa:group(accounts,"empresaNome","valorCentavos"),porCentroCusto:group(accounts,"centroCustoNome","valorCentavos")};
  }

  global.IntegroControleFinanceiro = Object.freeze({
    COLLECTIONS, MAX_FILE_BYTES, normalizeStatus, listarContas, listarPagamentos, listarFornecedores, listarCategorias, listarCentrosCusto, listarEmpresas, listarContasBancarias, listarRecorrencias, listarLembretes, listarAuditoria,
    ...
    criarParcelamento, criarRecorrencia, gerarOcorrenciasRecorrencia, salvarLembretes, anexarArquivo, removerArquivo, resumoRelatorios, recurrenceNext
  });
})(window);
