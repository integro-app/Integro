...
(function () {
  ...
  const STATUS_ATIVOS = ["RECEBIDA", "ATRIBUIDA", "EM_ATENDIMENTO", "DEVOLVIDA"];
  ...
  const TRANSICOES_STATUS = {
    ...
    ATRIBUIDA: ["ATRIBUIDA", "EM_ATENDIMENTO", "DEVOLVIDA", "RECUSADA", "CANCELADA"],
    EM_ATENDIMENTO: ["DEVOLVIDA", "NAO_CONVERTIDA", "RECUSADA", "CONVERTIDA", "CANCELADA"],
    DEVOLVIDA: ["ATRIBUIDA", "RECUSADA", "CANCELADA"],
    NAO_CONVERTIDA: [],
    RECUSADA: [],
    CONVERTIDA: [],
    DUPLICADA: [],
  ...
  function usuarioNoEscopoIndicacao(usuario = {}, indicacao = {}) {
    ...
    if (acesso.cargoChave === "vendedor") {
      const authUidCanonico = texto(indicacao.vendedorAuthUid || indicacao.vendedorDestinoAuthUid || indicacao.vendedorUid);
      if (authUidCanonico) return idsUsuario.includes(authUidCanonico);
      return idsUsuario.some(id => vendedoresIndicacao.includes(id));
    }
  ...
  function montarIndicacao(dados = {}, cliente = {}, usuario = {}) {
    ...
    return {
      ...
      indicadoPorAuthUid: authUidUsuario(usuario),
      ...
      vendedorDestinoAuthUid: texto(dados.vendedorAuthUid || dados.vendedorUid || dados.vendedorDestinoAuthUid),
      vendedorId,
      ...
      statusLead: statusIndicacao === "ATRIBUIDA" ? "NOVO_LEAD" : statusIndicacao,
  ...
  async function resolverVendedorCanonico(db, referencia = {}) {
    if (!db) return null;
    const tenant = texto(referencia.clientePlataformaId || referencia.tenantId || referencia.empresaId);
    const documentos = [...new Set([
      referencia.vendedorDocumentoId,
      referencia.vendedorDestinoId,
      referencia.vendedorId
    ].filter(Boolean).map(String))];

    for (const documentoId of documentos) {
      try {
        const snap = await db.collection("usuarios").doc(documentoId).get();
        if (!snap.exists) continue;
        const usuario = { id:snap.id || documentoId, ...snap.data() };
        if (tenant && tenantUsuario(usuario) && tenantUsuario(usuario) !== tenant) continue;
        const authUid = texto(usuario.authUid || usuario.uid);
        if (authUid) return { documentoId:String(snap.id || documentoId), authUid, usuario };
      } catch (_) {}
    }

    const authCandidates = [...new Set([
      referencia.vendedorAuthUid,
      referencia.vendedorDestinoAuthUid,
      referencia.vendedorUid,
      referencia.destinatarioAuthUid
    ].filter(Boolean).map(String))];
    for (const authUid of authCandidates) {
      try {
        const busca = await db.collection("usuarios").where("authUid", "==", authUid).limit(2).get();
        if (busca.empty || busca.docs.length !== 1) continue;
        const snap = busca.docs[0];
        const usuario = { id:snap.id, ...snap.data() };
        if (tenant && tenantUsuario(usuario) && tenantUsuario(usuario) !== tenant) continue;
        return { documentoId:String(snap.id), authUid:texto(usuario.authUid || authUid), usuario };
      } catch (_) {}
    }
    return null;
  }
  ...
  async function registrarNotificacaoIndicacao(db, dados = {}) {
    const tipo = texto(dados.tipo || "INDICACAO_ATUALIZADA").toUpperCase();
    const ehNovoLead = tipo === "NOVO_LEAD";
    const destinatarioAuthUid = texto(dados.destinatarioAuthUid || (!ehNovoLead ? (dados.vendedorAuthUid || dados.destinatarioId || dados.vendedorId) : ""));
    const notificacaoCriadorLead = texto(dados.destinatarioTipo).toUpperCase() === "CRIADOR_LEAD";
    const vendedorAuthUid = notificacaoCriadorLead ? "" : texto(dados.vendedorAuthUid || destinatarioAuthUid);
    const vendedorDocumentoId = notificacaoCriadorLead ? "" : texto(dados.vendedorDocumentoId || dados.vendedorDestinoId || dados.vendedorId);
    if (!db || !destinatarioAuthUid || !dados.indicacaoId) return false;
    ...
    await db.collection("notificacoes").doc(notificacaoId).set({
      ...
      // O Auth UID é a única chave operacional de destinatário.
      usuarioId: destinatarioAuthUid,
      destinatarioId: destinatarioAuthUid,
      destinatarioAuthUid,
      destinatarioTipo: dados.destinatarioTipo || "DIRECIONADA",
      publico: dados.publico || "",
      // IDs de vendedor abaixo são apenas metadados/auditoria; não definem destinatário.
      vendedorId: vendedorDocumentoId || vendedorAuthUid,
      vendedorDestinoId: vendedorDocumentoId || vendedorAuthUid,
      vendedorAuthUid,
      vendedorDocumentoId,
      origemTipo: "INDICACAO",
      origemId: dados.indicacaoId,
      acaoTipo: "ABRIR_INDICACAO",
  ...
  async function garantirNotificacaoLeadAtribuido(indicacao = {}, usuario = {}, opcoes = {}) {
    const db = opcoes.db || usuario.db || getDb();
    const authUidAtual = authUidUsuario(usuario);
    const status = normalizarStatusIndicacao(indicacao.statusIndicacao || indicacao.status);
    if (!db || !authUidAtual || !indicacao.id || !["ATRIBUIDA", "EM_ATENDIMENTO"].includes(status)) return false;

    const destino = await resolverVendedorCanonico(db, indicacao);
    if (!destino?.authUid || destino.authUid !== authUidAtual) return false;

    const notificacaoId = idNotificacaoIndicacao(["indicacao", "NOVO_LEAD", indicacao.id, destino.authUid]);
    const existente = await db.collection("notificacoes").doc(notificacaoId).get();
    ...
    await registrarNotificacaoIndicacao(db, {
      ...
      destinatarioAuthUid:destino.authUid,
      vendedorAuthUid:destino.authUid,
      vendedorDocumentoId:destino.documentoId,
      criadoPorId:texto(indicacao.indicadoPorId || indicacao.criadoPorId),
      criadoPorAuthUid:texto(indicacao.indicadoPorAuthUid || indicacao.criadoPorAuthUid),
      criadoPorNome:texto(indicacao.indicadoPorNome || indicacao.criadoPorNome)
    });
    return true;
  }
  ...
  async function atualizarStatusIndicacao(indicacaoId, dados = {}, usuario = {}, tipoLog = "INDICACAO_ATUALIZADA") {
    ...
    const agoraTexto = window.IntegroOperacional?.dataHoraSP?.() || new Date().toISOString();
    const statusLeadNovo = normalizarStatusIndicacao(statusNovo) === "ATRIBUIDA"
      ? (texto(dados.statusLead) || "NOVO_LEAD")
      : normalizarStatusIndicacao(statusNovo);
    const payload = {
      ...dados,
      statusLead: statusLeadNovo,
      db: undefined,
    ...
    const mensagensStatus = {
      ...
      DEVOLVIDA:["Lead devolvido",`${atual.nomeClienteSnapshot || atual.nome || "O lead"} foi devolvido ao setor de leads por ${nomeUsuario(usuario) || "um vendedor"}.`],
  ...
  async function atribuirIndicacao(indicacaoId, destino = {}, usuario = {}) {
    ...
    const vendedorDestino = await validarDestinoVendedorIndicacao(db, destino, usuario, indicacaoAtual);
    await validarClienteSemHistoricoParaAtribuicao(db, indicacaoAtual, usuario);
    const vendedorDocumentoId = texto(vendedorDestino?.id || destino.vendedorDestinoId || destino.vendedorId);
    const vendedorAuthUidValidado = texto(vendedorDestino?.authUid || vendedorDestino?.uid || destino.vendedorAuthUid || destino.vendedorUid || destino.vendedorDestinoAuthUid || vendedorDocumentoId);
    const vendedorNomeValidado = texto(destino.vendedorDestinoNome || destino.vendedorNome || vendedorDestino?.nome || vendedorDestino?.nomeCompleto || vendedorDestino?.email);
    const equipeIdValidada = texto(destino.equipeDestinoId || destino.equipeId || vendedorDestino?.equipeId || vendedorDestino?.equipesIds?.[0] || vendedorDestino?.equipeIds?.[0]);
    await atualizarStatusIndicacao(indicacaoId, {
      vendedorDestinoId: vendedorDocumentoId,
      vendedorDestinoNome: vendedorNomeValidado,
      vendedorAuthUid: vendedorAuthUidValidado,
      vendedorId: vendedorDocumentoId,
      vendedorNome: vendedorNomeValidado,
      equipeDestinoId: equipeIdValidada,
      equipeDestinoNome: destino.equipeDestinoNome || destino.equipeNome || vendedorDestino?.equipeNome || "",
      db: destino.db,
      statusIndicacao: "ATRIBUIDA",
      status: "ATRIBUIDA",
    ...
    const vendedorId = vendedorDocumentoId;
    const vendedorAuthUid = vendedorAuthUidValidado;
  ...
  async function salvarEdicaoIndicacao(indicacaoId, dados = {}, usuario = {}) {
    ...
    let vendedorDestinoValidado = null;
    if (vendedorAlterado) {
      vendedorDestinoValidado = await validarDestinoVendedorIndicacao(db, dados, usuario, atual);
      await validarClienteSemHistoricoParaAtribuicao(db, atual, usuario);
    }
    const vendedorAuthUidNovo = vendedorAlterado
      ? texto(vendedorDestinoValidado?.authUid || vendedorDestinoValidado?.uid || dados.vendedorAuthUid || dados.vendedorUid || vendedorId)
      : texto(dados.vendedorAuthUid || dados.vendedorUid || atual.vendedorAuthUid);
    ...
    await atualizarStatusIndicacao(indicacaoId, {
      ...
      vendedorAuthUid:vendedorAuthUidNovo,
      vendedorDestinoAuthUid:vendedorAuthUidNovo,
      vendedorDocumentoId:vendedorId,
      vendedorId,
      ...
      status:statusNovo,
      statusLead:vendedorAlterado ? "NOVO_LEAD" : (statusNovo === "ATRIBUIDA" ? (atual.statusLead || "NOVO_LEAD") : statusNovo),
    ...
    if (atual.clienteOperacionalId) {
      ...
      await clienteRef.set({
        ...
        whatsappUrl,
        ...(vendedorAlterado ? {
          vendedorId:vendedorAuthUidNovo,
          vendedorAuthUid:vendedorAuthUidNovo,
          vendedorDocumentoId:vendedorId,
          vendedorNome:texto(dados.vendedorDestinoNome || dados.vendedorNome),
    ...
    if (vendedorAlterado) {
      const vendedorAuthUid = vendedorAuthUidNovo;
      await registrarNotificacaoIndicacao(db, {
        tipo:"NOVO_LEAD",
        titulo:"Você recebeu um novo lead",
  ...
  async function devolverIndicacaoAoSetor(indicacaoId, motivo, usuario = {}) {
    if (!texto(motivo)) throw new Error("Informe o motivo da devolução.");
    const db = usuario.db || getDb();
    const ref = db.collection("indicacoes").doc(indicacaoId);
    const snap = await ref.get();
    if (!snap.exists) throw new Error("Indicação não encontrada.");
    const atual = { id: snap.id || indicacaoId, ...snap.data() };
    const agora = window.IntegroOperacional?.dataHoraSP?.() || new Date().toISOString();
    await atualizarStatusIndicacao(indicacaoId, {
      statusIndicacao: "DEVOLVIDA",
      status: "DEVOLVIDA",
      motivoDevolucao: texto(motivo),
      dataDevolucao: agora,
      vendedorDestinoId: "",
      vendedorDestinoNome: "",
      vendedorAuthUid: "",
      vendedorDestinoAuthUid: "",
      vendedorUid: "",
      vendedorDocumentoId: "",
      vendedorId: "",
      vendedorNome: "",
      ...
      equipeNome: "",
      __permissaoIndicacao: "podeReceberIndicacoes"
    }, usuario, "INDICACAO_DEVOLVIDA");
    if (atual.clienteOperacionalId) {
      await db.collection("clientes_operacionais").doc(atual.clienteOperacionalId).set({
        ...
        atualizadoPor: idUsuario(usuario), atualizadoPorNome: nomeUsuario(usuario),
        atualizadoEmTexto: agora, atualizadoEm: serverTimestamp()
      }, { merge: true });
    }
    return true;
  }
  ...
  function calcularDashboardIndicacoes(indicacoes = []) {
    ...
    return {
      ...
      novosLeads: contar("ATRIBUIDA"),
      atribuidas: contar("ATRIBUIDA"),
      devolvidas: contar("DEVOLVIDA"),
  ...
  window.IntegroIndicacoes = {
    ...
    garantirNotificacaoLeadAtribuido,
