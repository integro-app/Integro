(function (global) {
  ...
  async function saveBackend(clientePlataformaId, entrada, usuario = {}) {
    ...
    const callable = global.firebase.app().functions("southamerica-east1").httpsCallable("salvarConfiguracoesEmpresaV27");
  ...
  function protectSensitiveCompanyFields() {
    const input = document.getElementById("configEmpresaNome") || document.getElementById("cfgEmpresaNomeSensivel");
    if (input) {
      input.disabled = true;
      input.title = "Dados cadastrais sensíveis são alterados somente pelo suporte ÍNTEGRO.";
      const label = input.closest("label");
      if (label && !label.querySelector("[data-v27-support-note]")) label.insertAdjacentHTML("beforeend", '<small data-v27-support-note>Alteração cadastral via suporte ÍNTEGRO.</small>');
    }
  }
  function install() {
    ...
    document.addEventListener("integro-tela-alterada", event => {
      if (event.detail?.tela === "configuracoes") protectSensitiveCompanyFields();
    });
    document.addEventListener("integro-configuracoes-atualizadas", protectSensitiveCompanyFields);
    return true;
  }
  global.IntegroV27ConfigSaveGuard = Object.freeze({ install, saveBackend, protectSensitiveCompanyFields });
  if (global.IntegroConfiguracoesEmpresa) install();
})(window);
