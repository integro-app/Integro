...
const FirestoreService = {
  normalizarDadosAcessoUsuario(tipoUsuario, cargo = null) {
    ...
    const cargosCliente = ["gerente", "captador", "supervisor", "vendedor", "financeiro", "administrativo", "auditor"];
  ...
  // ===============================
  async loadCollection(collectionName, tenantId = null, limit = null, options = {}) {
    try {
      ...
      if (tenantId && runtime?.consultarTenant) {
        return runtime.consultarTenant({
          ...
          limite: limiteQuery,
          cacheMs,
          forcar: options?.forcar === true,
          chave: options?.chave || `collection:${colecao}:${tenantId}:${limiteQuery}`
        });
      }

      let ref = db.collection(colecao);
      if (tenantId) ref = ref.where(CONFIG.TENANT_ID_KEY, "==", tenantId);
  ...
  async loadCollectionCached(collectionName, tenantId = null, limit = null, cacheMs = 30000, forcar = false) {
    return this.loadCollection(collectionName, tenantId, limit, { cacheMs, forcar });
  },

  invalidateCollection(collectionName = "") {
    const colecao = collectionName
      ? (CONFIG.COLECOES[String(collectionName).toUpperCase()] || collectionName)
      : "";
    window.IntegroDataRuntime?.invalidar?.(colecao);
  },
