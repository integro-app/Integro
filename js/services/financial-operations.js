...
(function () {
  ...
  function idSeguroNotificacao(valor) {
    return texto(valor).replace(/[^a-zA-Z0-9_-]/g, "_").slice(0, 220);
  }

  function referenciaNotificacaoSolicitacao(db, solicitacaoId, status) {
    return db.collection("notificacoes").doc(idSeguroNotificacao(`solicitacao_${solicitacaoId}_${status}`));
  }

  function payloadNotificacaoSolicitacao(solicitacao = {}, caixa = {}, status = "", mensagem = "") {
    const statusNormalizado = normalizarStatus(status);
    const aprovado = statusNormalizado.includes("APROV");
    const destinatarioAuthUid = texto(
      solicitacao.vendedorAuthUid || solicitacao.vendedorUid || solicitacao.solicitanteAuthUid ||
      solicitacao.solicitanteId || caixa.vendedorAuthUid || caixa.vendedorUid || solicitacao.criadoPorId || solicitacao.vendedorId
    );
    const vendedorId = texto(solicitacao.vendedorId || caixa.vendedorId || destinatarioAuthUid);
    const tipoMov = tipoMovimentoCaixa(solicitacao) || texto(solicitacao.tipoSolicitacao || solicitacao.tipo || "MOVIMENTACAO");
    const titulo = aprovado ? "Movimentação aprovada" : "Movimentação recusada";
    return {
      clientePlataformaId: texto(solicitacao.clientePlataformaId || caixa.clientePlataformaId),
      tenantId: texto(solicitacao.clientePlataformaId || caixa.clientePlataformaId),
      tipo: aprovado ? "MOVIMENTACAO_APROVADA" : "MOVIMENTACAO_RECUSADA",
      categoria: "MOVIMENTACOES",
      prioridade: aprovado ? "NORMAL" : "ALTA",
      titulo,
      mensagem: texto(mensagem) || (aprovado ? `${tipoMov} aprovado pelo financeiro.` : `${tipoMov} recusado pelo financeiro.`),
      solicitacaoId: texto(solicitacao.id || solicitacao.solicitacaoId),
      movimentacaoId: texto(solicitacao.id || solicitacao.solicitacaoId),
      caixaId: texto(solicitacao.caixaId || caixa.id),
      ...
      origemEvento: aprovado ? "MOVIMENTACAO_APROVADA" : "MOVIMENTACAO_RECUSADA",
      origemTipo: "MOVIMENTACAO",
      origemId: texto(solicitacao.id || solicitacao.solicitacaoId),
      entidadeTipo: "MOVIMENTACAO",
      entidadeId: texto(solicitacao.id || solicitacao.solicitacaoId),
      eventoId: `${aprovado ? "APROVACAO" : "RECUSA"}:${texto(solicitacao.id || solicitacao.solicitacaoId)}:${statusNormalizado}`,
      idempotencyKey: `${aprovado ? "MOVIMENTACAO_APROVADA" : "MOVIMENTACAO_RECUSADA"}:${texto(solicitacao.id || solicitacao.solicitacaoId)}:${destinatarioAuthUid}`,
      rota: { tela:"movimentacoes", entidadeId:texto(solicitacao.id || solicitacao.solicitacaoId), acao:"ABRIR_DETALHE" },
      acaoTipo: "ABRIR_MOVIMENTACAO",
      usuarioId: destinatarioAuthUid,
      destinatarioId: destinatarioAuthUid,
      destinatarioAuthUid,
      destinatarioTipo: "VENDEDOR",
      vendedorId,
      vendedorAuthUid: destinatarioAuthUid,
      publico: "",
      lida: false,
      excluido: false,
      excluida: false,
  ...
  function payloadLancamentoFinanceiro(entrada = {}, extra = {}) {
    ...
    return {
      ...
      vendedorId: texto(entrada.vendedorId || caixa.vendedorId || usuario.vendedorId || usuario.id || usuario.usuarioId || ""),
      vendedorAuthUid: texto(entrada.vendedorAuthUid || caixa.vendedorAuthUid || caixa.vendedorUid || caixa.uid || usuario.authUid || usuario.uid || ""),
      vendedorNome: texto(entrada.vendedorNome || caixa.vendedorNome || usuario.nome || usuario.nomeCompleto || usuario.email || ""),
      equipeId: texto(entrada.equipeId || caixa.equipeId || caixa.equipeUid || caixa.unidadeId || usuario.equipeId || usuario.equipesIds?.[0] || usuario.equipeIds?.[0] || ""),
      equipeNome: texto(entrada.equipeNome || caixa.equipeNome || caixa.nomeEquipe || caixa.unidadeNome || usuario.equipeNome || usuario.nomeEquipe || ""),
      supervisorId: texto(entrada.supervisorId || caixa.supervisorId || caixa.responsavelId || ""),
      gerenteId: texto(entrada.gerenteId || caixa.gerenteId || ""),
      categoriaId: texto(entrada.categoriaId || entrada.metadados?.categoriaId || ""),
      categoriaNome: texto(entrada.categoriaNome || entrada.metadados?.categoriaNome || ""),
      categoriaTipo: texto(entrada.categoriaTipo || entrada.metadados?.categoriaTipo || ""),
      tipoLancamento: validado.tipoLancamento,
    ...
    return db.runTransaction(async transaction => {
      ...
      if (caixaSnap?.exists && ["INGRESSO", "GASTO", "RETIRADA"].includes(tipo)) {
        ...
        if (perfilOperador.isVendedor) {
          validarVendedorCaixa(caixa, usuario);
          if (tipo === "INGRESSO") {
            throw new Error("Ingressos do vendedor devem ser enviados como solicitação ao financeiro.");
          }
        } else if (!usuarioAdministradorFinanceiro(usuario, caixa, entrada)) {
          throw new Error("Usuário sem permissão para criar movimentações neste caixa.");
        }
      }
      ...
      const entradaEfetiva = perfilOperador.isVendedor && ["GASTO", "RETIRADA"].includes(tipo)
        ? {
            ...entrada,
            lancamentoId,
            vendedorId: texto(caixa.vendedorId || caixa.usuarioId || entrada.vendedorId),
            vendedorAuthUid: texto(caixa.vendedorAuthUid || caixa.vendedorUid || caixa.uid || entrada.vendedorAuthUid),
            equipeId: texto(caixa.equipeId || entrada.equipeId)
          }
        : { ...entrada, lancamentoId };
      const payload = payloadLancamentoFinanceiro(entradaEfetiva, { caixa });
  ...
  async function calcularResumoFinanceiroPeriodo(opcoes = {}) {
    ...
    lista.forEach(l => {
      const impacto = impactoLancamentoCentavos(l);
      if (impacto === 0) return;
      const valor = Math.abs(impacto);
  ...
  async function registrarLancamentoSolicitacaoFinanceiraTransacional(entrada = {}) {
    ...
    return db.runTransaction(async transaction => {
      ...
      const statusSolicitacao = normalizarStatus(solicitacao.statusSolicitacao || solicitacao.status || "PENDENTE");
      if (!["PENDENTE", "APROVADA", "APROVADO"].includes(statusSolicitacao)) throw new Error("A solicitação não está disponível para aprovação.");
      ...
      const caixaRef = caixaId ? db.collection("caixas").doc(caixaId) : null;
      const caixaSnap = caixaRef ? await transaction.get(caixaRef) : null;
      if (!caixaId || !caixaSnap?.exists) throw new Error("Caixa da solicitação não encontrado.");
      const caixa = { id: caixaId, ...caixaSnap.data() };
      validarTenant(caixa, tenantId, "Caixa");
      const statusCaixa = normalizarStatus(caixa.status || caixa.statusCaixa);
      if (!["ABERTO", "REABERTO"].includes(statusCaixa)) throw new Error("O ingresso só pode ser aprovado enquanto o caixa estiver aberto.");
      if (!usuarioPodeAprovarSolicitacaoFinanceira(usuario, caixa, entrada)) {
        throw new Error("Usuário sem permissão para aprovar solicitações financeiras.");
      }
      ...
      if (lancamentoSnap.exists) {
        if (statusSolicitacao === "PENDENTE" && entrada.marcarAprovada !== false) {
          const respostaAprovacao = texto(entrada.resposta || entrada.observacao || "Solicitação aprovada.");
          transaction.update(solicitacaoRef, {
            status: "APROVADA",
            statusSolicitacao: "APROVADA",
            lancamentoFinanceiroId: lancamentoId,
            aprovadoPor: texto(usuario.id || usuario.usuarioId || usuario.authUid || usuario.uid),
            aprovadoPorNome: texto(usuario.nome || usuario.nomeCompleto || usuario.email),
            analisadoPor: texto(usuario.id || usuario.usuarioId || usuario.authUid || usuario.uid),
            analisadoPorNome: texto(usuario.nome || usuario.nomeCompleto || usuario.email),
            respostaFinanceiro: respostaAprovacao,
            analisadoEm: serverTimestamp(),
            atualizadoEm: serverTimestamp()
          });
          const notifRef = referenciaNotificacaoSolicitacao(db, solicitacaoId, "APROVADA");
          transaction.set(notifRef, payloadNotificacaoSolicitacao(solicitacao, caixa, "APROVADA", respostaAprovacao), { merge: true });
        }
        return { modo: "IDEMPOTENTE", lancamentoId, solicitacaoId };
      }
      const valorCentavos = valorMovimentoCentavos(solicitacao);
      if (valorCentavos <= 0) throw new Error("Valor da solicitação deve ser maior que zero.");
      const payload = payloadLancamentoFinanceiro({
        tipoLancamento,
        natureza,
        ...
        categoriaNome: solicitacao.categoriaNome || solicitacao.categoria || "",
        categoriaTipo: solicitacao.categoriaTipo || tipoLancamento,
        valorCentavos,
      ...
      if (["INGRESSO", "GASTO", "RETIRADA"].includes(payload.tipoLancamento)) {
        const deltaCentavos = impactoLancamentoCentavos(payload);
        const saldoAtualCentavos = centavosDe(caixa, "saldoAtualCentavos", ["saldoAtual", "valorAtual", "caixaAtual", "saldo"]);
        const campoTotal = payload.tipoLancamento === "INGRESSO" ? "totalIngressosCentavos"
          : payload.tipoLancamento === "GASTO" ? "totalGastosCentavos"
        ...
        transaction.update(caixaRef, {
          ...
          valorAtual: reais(novoSaldoCentavos),
          caixaAtual: reais(novoSaldoCentavos),
          [campoTotal]: Math.round(Number(caixa[campoTotal] || 0)) + payload.valorCentavos,
          atualizadoEm: serverTimestamp()
        });
      }
      ...
      transaction.set(db.collection("logs").doc(), {
        ...
        tipoLancamento,
        valorCentavos: payload.valorCentavos,
        usuarioId: texto(usuario.id || usuario.usuarioId || usuario.authUid || usuario.uid),
        criadoEm: serverTimestamp()
      });
  ...
  async function recusarSolicitacaoFinanceiraTransacional(entrada = {}) {
    const db = getDb();
    const usuario = entrada.usuario || {};
    const solicitacaoId = texto(entrada.solicitacaoId || entrada.origemId);
    const tenantId = texto(entrada.clientePlataformaId || usuario.clientePlataformaId || usuario.tenantId || usuario.empresaId);
    const motivo = texto(entrada.motivo || entrada.resposta || entrada.justificativa);
    if (!solicitacaoId || !tenantId) throw new Error("Solicitação e tenant são obrigatórios.");
    if (!motivo) throw new Error("Motivo da recusa obrigatório.");
    const solicitacaoRef = db.collection("solicitacoes").doc(solicitacaoId);
    return db.runTransaction(async transaction => {
      const solicitacaoSnap = await transaction.get(solicitacaoRef);
      if (!solicitacaoSnap.exists) throw new Error("Solicitação não encontrada.");
      const solicitacao = { id: solicitacaoId, ...solicitacaoSnap.data() };
      validarTenant(solicitacao, tenantId, "Solicitação");
      const status = normalizarStatus(solicitacao.statusSolicitacao || solicitacao.status || "PENDENTE");
      if (status === "RECUSADA") return { modo: "IDEMPOTENTE", solicitacaoId, status: "RECUSADA" };
      if (status !== "PENDENTE") throw new Error("Somente solicitações pendentes podem ser recusadas.");
      const caixaId = texto(entrada.caixaId || solicitacao.caixaId || solicitacao.idCaixa || solicitacao.caixaAtualId);
      let caixa = {
        id: caixaId,
        clientePlataformaId: tenantId,
        equipeId: solicitacao.equipeId || "",
        vendedorId: solicitacao.vendedorId || "",
        vendedorAuthUid: solicitacao.vendedorAuthUid || ""
      };
      if (caixaId) {
        const caixaSnap = await transaction.get(db.collection("caixas").doc(caixaId));
        if (caixaSnap.exists) {
          caixa = { id: caixaId, ...caixaSnap.data() };
          validarTenant(caixa, tenantId, "Caixa");
        }
      }
      if (!usuarioPodeAprovarSolicitacaoFinanceira(usuario, caixa, entrada)) {
        throw new Error("Usuário sem permissão para recusar solicitações financeiras.");
      }
      const analisadoPor = texto(usuario.id || usuario.usuarioId || usuario.authUid || usuario.uid);
      const analisadoPorNome = texto(usuario.nome || usuario.nomeCompleto || usuario.email);
      transaction.update(solicitacaoRef, {
        status: "RECUSADA",
        statusSolicitacao: "RECUSADA",
        respostaFinanceiro: motivo,
        motivoRecusa: motivo,
        analisadoPor,
        analisadoPorNome,
        recusadoPor: analisadoPor,
        recusadoPorNome: analisadoPorNome,
        analisadoEm: serverTimestamp(),
        recusadoEm: serverTimestamp(),
        atualizadoEm: serverTimestamp()
      });
      const notifRef = referenciaNotificacaoSolicitacao(db, solicitacaoId, "RECUSADA");
      transaction.set(notifRef, payloadNotificacaoSolicitacao(solicitacao, caixa, "RECUSADA", motivo), { merge: true });
      transaction.set(db.collection("logs").doc(), {
        tipoAcao: "SOLICITACAO_FINANCEIRA_RECUSADA",
        clientePlataformaId: tenantId,
        caixaId,
        solicitacaoId,
        tipoSolicitacao: tipoMovimentoCaixa(solicitacao),
        valorCentavos: valorMovimentoCentavos(solicitacao),
        motivo,
        usuarioId: analisadoPor,
        usuarioNome: analisadoPorNome,
        criadoEm: serverTimestamp()
      });
      return { modo: "ATUALIZACAO", solicitacaoId, status: "RECUSADA" };
    });
  }

  async function cancelarSolicitacaoFinanceiraPendenteTransacional(entrada = {}) {
    const db = getDb();
    const usuario = entrada.usuario || {};
    ...
    if (!solicitacaoId || !tenantId) throw new Error("Solicitação e tenant são obrigatórios.");
    if (!motivo) throw new Error("Motivo do cancelamento obrigatório.");
    const solicitacaoRef = db.collection("solicitacoes").doc(solicitacaoId);
    return db.runTransaction(async transaction => {
      const solicitacaoSnap = await transaction.get(solicitacaoRef);
      if (!solicitacaoSnap.exists) throw new Error("Solicitação não encontrada.");
      const solicitacao = { id: solicitacaoId, ...solicitacaoSnap.data() };
      validarTenant(solicitacao, tenantId, "Solicitação");
      if (normalizarStatus(solicitacao.statusSolicitacao || solicitacao.status) !== "PENDENTE") throw new Error("Somente solicitações pendentes podem ser canceladas.");
      validarVendedorRegistro(solicitacao, usuario, "Solicitação");
      const caixaIdSolicitacao = texto(solicitacao.caixaId || solicitacao.idCaixa || solicitacao.caixaAtualId);
      const caixaId = texto(entrada.caixaId || caixaIdSolicitacao);
      if (!caixaIdSolicitacao || caixaId !== caixaIdSolicitacao) {
        throw new Error("A solicitação não pertence ao caixa informado.");
      }
      const caixaRef = db.collection("caixas").doc(caixaId);
      const caixaSnap = await transaction.get(caixaRef);
      if (!caixaSnap.exists) throw new Error("Caixa da solicitação não encontrado.");
      const caixa = { id: caixaId, ...caixaSnap.data() };
      validarTenant(caixa, tenantId, "Caixa");
      validarVendedorCaixa(caixa, usuario);
      if (!["ABERTO", "REABERTO"].includes(normalizarStatus(caixa.status))) throw new Error("A solicitação pertence a um caixa que não está aberto.");
      transaction.update(solicitacaoRef, {
        status: "CANCELADA",
        statusSolicitacao: "CANCELADA",
        canceladoPorId: texto(usuario.authUid || usuario.uid || usuario.id || usuario.usuarioId),
        canceladoPorNome: texto(usuario.nome || usuario.nomeCompleto || usuario.email),
        motivoCancelamento: motivo,
        canceladoEm: serverTimestamp(),
        atualizadoEm: serverTimestamp()
      });
      transaction.set(db.collection("logs").doc(), {
        tipoAcao: "SOLICITACAO_FINANCEIRA_CANCELADA_PELO_VENDEDOR",
        clientePlataformaId: tenantId,
        caixaId,
        solicitacaoId,
        motivo,
        usuarioId: texto(usuario.authUid || usuario.uid || usuario.id || usuario.usuarioId),
        criadoEm: serverTimestamp()
      });
      return { solicitacaoId, status: "CANCELADA" };
    });
  }

  function usuarioPodeAprovarSolicitacaoFinanceira(usuario = {}, caixa = {}, entrada = {}) {
    const perfil = perfilAcessoCaixa(usuario);
    const tipo = normalizarStatus(usuario.tipoUsuario || usuario.tipo || usuario.role || usuario.perfil);
    const cargo = normalizarStatus(usuario.cargoChave || usuario.cargo || usuario.cargoNome);
    if (perfil.isMasterGlobal || perfil.isMasterLocal || tipo === "FINANCEIRO" || cargo === "FINANCEIRO") return true;
    if (perfil.isSupervisor && !supervisorNoEscopoCaixa(usuario, caixa) && entrada.permissaoAdministrativa !== true) return false;
    return usuario?.permissoes?.solicitacoes?.aprovar === true ||
      usuario?.permissoes?.financeiro?.aprovar === true ||
      usuario?.permissoes?.financeiro?.aprovarIngresso === true ||
      usuario?.permissoes?.caixas?.aprovar === true ||
      usuarioPodeFinanceiro(usuario, "aprovar", caixa, entrada);
  }

  function usuarioAdministradorFinanceiro(usuario = {}, caixa = {}, entrada = {}) {
    const perfil = perfilAcessoCaixa(usuario);
    const tipo = normalizarStatus(usuario.tipoUsuario || usuario.tipo || usuario.role || usuario.perfil);
    const cargo = normalizarStatus(usuario.cargoChave || usuario.cargo || usuario.cargoNome);
    if (perfil.isMasterGlobal || perfil.isMasterLocal || tipo === "FINANCEIRO" || cargo === "FINANCEIRO") return true;
    return usuarioPodeFinanceiro(usuario, "podeEstornarLancamento", caixa, entrada) ||
      usuarioPodeFinanceiro(usuario, "podeCriarLancamentoFinanceiro", caixa, entrada) ||
      usuarioPodeFinanceiro(usuario, "criarLancamento", caixa, entrada);
  }

  async function cancelarLancamentoFinanceiroCaixaAbertoTransacional(entrada = {}) {
    const db = getDb();
    const usuario = entrada.usuario || {};
    const lancamentoId = texto(entrada.lancamentoId);
    const tenantId = texto(entrada.clientePlataformaId || usuario.clientePlataformaId || usuario.tenantId || usuario.empresaId);
    const motivo = texto(entrada.motivo);
    if (!lancamentoId || !tenantId) throw new Error("Lançamento e tenant são obrigatórios.");
    if (!motivo) throw new Error("Motivo do cancelamento obrigatório.");
    const lancamentoRef = db.collection("lancamentos_financeiros").doc(lancamentoId);
    return db.runTransaction(async transaction => {
      const lancamentoSnap = await transaction.get(lancamentoRef);
      if (!lancamentoSnap.exists) throw new Error("Lançamento financeiro não encontrado.");
      const lancamento = { id: lancamentoId, ...lancamentoSnap.data() };
      validarTenant(lancamento, tenantId, "Lançamento financeiro");
      if (normalizarStatus(lancamento.statusLancamento || "CONFIRMADO") !== "CONFIRMADO") throw new Error("Somente lançamentos confirmados podem ser cancelados.");
      const caixaIdLancamento = texto(lancamento.caixaId);
      const caixaId = texto(entrada.caixaId || caixaIdLancamento);
      if (!caixaIdLancamento || caixaId !== caixaIdLancamento) {
        throw new Error("O lançamento não pertence ao caixa informado.");
      }
      const caixaRef = db.collection("caixas").doc(caixaId);
      const caixaSnap = await transaction.get(caixaRef);
      if (!caixaSnap.exists) throw new Error("Caixa do lançamento não encontrado.");
      const caixa = { id: caixaId, ...caixaSnap.data() };
      validarTenant(caixa, tenantId, "Caixa");
      const statusCaixa = normalizarStatus(caixa.status || caixa.statusCaixa);
      if (!["ABERTO", "REABERTO"].includes(statusCaixa)) throw new Error("O caixa precisa estar aberto ou reaberto para cancelar o lançamento.");
      const perfil = perfilAcessoCaixa(usuario);
      ...
      const impactoOriginal = impactoLancamentoCentavos(lancamento);
      const saldoAtualCentavos = centavosDe(caixa, "saldoAtualCentavos", ["saldoAtual", "valorAtual", "caixaAtual", "saldo"]);
      const novoSaldoCentavos = saldoAtualCentavos - impactoOriginal;
      const tipo = normalizarStatus(lancamento.tipoLancamento);
      const campoTotal = tipo === "INGRESSO" ? "totalIngressosCentavos" : tipo === "GASTO" ? "totalGastosCentavos" : tipo === "RETIRADA" ? "totalRetiradasCentavos" : "";
      const updateCaixa = {
        saldoAtualCentavos: novoSaldoCentavos,
        saldoAtual: reais(novoSaldoCentavos),
        valorAtual: reais(novoSaldoCentavos),
      ...
      if (campoTotal) updateCaixa[campoTotal] = Math.max(0, Math.round(Number(caixa[campoTotal] || 0)) - Math.abs(Math.round(Number(lancamento.valorCentavos || 0))));
      transaction.update(caixaRef, updateCaixa);
      ...
      transaction.set(db.collection("logs").doc(), {
        tipoAcao: "LANCAMENTO_FINANCEIRO_CANCELADO",
        clientePlataformaId: tenantId,
        caixaId,
        lancamentoId,
        tipoLancamento: tipo,
        valorCentavos: Math.abs(Math.round(Number(lancamento.valorCentavos || 0))),
        motivo,
        usuarioId: texto(usuario.authUid || usuario.uid || usuario.id || usuario.usuarioId),
        criadoEm: serverTimestamp()
      });
      return { lancamentoId, status: "CANCELADO", caixaId, novoSaldoCentavos };
    });
  }

  async function editarLancamentoFinanceiroAdministrativoTransacional(entrada = {}) {
    const db = getDb();
    const usuario = entrada.usuario || {};
    const lancamentoOriginalId = texto(entrada.lancamentoId || entrada.lancamentoOriginalId);
    const tenantId = texto(entrada.clientePlataformaId || usuario.clientePlataformaId || usuario.tenantId || usuario.empresaId);
    const motivo = texto(entrada.motivoEdicao || entrada.motivo || "Correção administrativa");
    const operacaoId = texto(entrada.operacaoId || `edicao_${lancamentoOriginalId}_${Date.now()}`);
    if (!lancamentoOriginalId || !tenantId) throw new Error("Lançamento original e tenant são obrigatórios.");
    const originalRef = db.collection("lancamentos_financeiros").doc(lancamentoOriginalId);
    return db.runTransaction(async transaction => {
      const originalSnap = await transaction.get(originalRef);
      if (!originalSnap.exists) throw new Error("Lançamento original não encontrado.");
      const original = { id: lancamentoOriginalId, ...originalSnap.data() };
      validarTenant(original, tenantId, "Lançamento financeiro");
      if (normalizarStatus(original.statusLancamento || "CONFIRMADO") !== "CONFIRMADO") throw new Error("Somente lançamentos confirmados podem ser editados.");
      const caixaId = texto(original.caixaId);
      const caixaRef = db.collection("caixas").doc(caixaId);
      const caixaSnap = await transaction.get(caixaRef);
      if (!caixaSnap.exists) throw new Error("Caixa do lançamento não encontrado.");
      const caixa = { id: caixaId, ...caixaSnap.data() };
      validarTenant(caixa, tenantId, "Caixa");
      if (!["ABERTO", "REABERTO"].includes(normalizarStatus(caixa.status))) throw new Error("Reabra o caixa antes de editar um lançamento histórico.");
      if (!usuarioAdministradorFinanceiro(usuario, caixa, entrada)) throw new Error("Usuário sem permissão para editar lançamentos.");
      const tipoNovo = normalizarStatus(entrada.tipoLancamento || original.tipoLancamento);
      if (!["INGRESSO", "GASTO", "RETIRADA"].includes(tipoNovo)) throw new Error("Tipo de lançamento não editável por este fluxo.");
      const valorNovoCentavos = Number.isInteger(entrada.valorCentavos) ? Math.abs(entrada.valorCentavos) : Math.abs(getOperacional().moedaParaCentavos(entrada.valor || 0));
      if (valorNovoCentavos <= 0) throw new Error("Valor editado deve ser maior que zero.");
      const origemId = `edicao_${lancamentoOriginalId}_${operacaoId}`;
      const novoId = lancamentoFinanceiroIdDeterministico({ tipoLancamento: tipoNovo, origemId, caixaId, operacaoId });
      const novoRef = db.collection("lancamentos_financeiros").doc(novoId);
      const novoSnap = await transaction.get(novoRef);
      if (novoSnap.exists) return { modo: "IDEMPOTENTE", lancamentoId: novoId, substituiLancamentoId: lancamentoOriginalId };
      const novoPayload = payloadLancamentoFinanceiro({
        ...original,
        ...entrada,
        lancamentoId: novoId,
        tipoLancamento: tipoNovo,
        natureza: tipoNovo === "INGRESSO" ? "CREDITO" : "DEBITO",
        origem: "EDICAO_ADMINISTRATIVA",
        origemId,
        operacaoId,
        ...
        caixaId,
        usuario,
        descricao: texto(entrada.descricao || original.descricao || "Lançamento corrigido administrativamente"),
        observacao: texto(entrada.observacao || original.observacao || motivo),
        metadados: { ...(original.metadados || {}), ...(entrada.metadados || {}), substituiLancamentoId: lancamentoOriginalId, motivoEdicao: motivo }
      }, { caixa });
      const impactoOriginal = impactoLancamentoCentavos(original);
      const impactoNovo = impactoLancamentoCentavos(novoPayload);
      const saldoAtualCentavos = centavosDe(caixa, "saldoAtualCentavos", ["saldoAtual", "valorAtual", "caixaAtual", "saldo"]);
      const novoSaldoCentavos = saldoAtualCentavos - impactoOriginal + impactoNovo;
      const updateCaixa = {
        saldoAtualCentavos: novoSaldoCentavos,
        saldoAtual: reais(novoSaldoCentavos),
        valorAtual: reais(novoSaldoCentavos),
      ...
      for (const tipo of ["INGRESSO", "GASTO", "RETIRADA"]) {
        const campo = tipo === "INGRESSO" ? "totalIngressosCentavos" : tipo === "GASTO" ? "totalGastosCentavos" : "totalRetiradasCentavos";
        let total = Math.round(Number(caixa[campo] || 0));
        if (normalizarStatus(original.tipoLancamento) === tipo) total -= Math.abs(Math.round(Number(original.valorCentavos || 0)));
        if (tipoNovo === tipo) total += novoPayload.valorCentavos;
        updateCaixa[campo] = Math.max(0, total);
      }
      transaction.update(caixaRef, updateCaixa);
      transaction.update(originalRef, {
        statusLancamento: "CANCELADO",
        substituidoPorLancamentoId: novoId,
        canceladoPorId: texto(usuario.authUid || usuario.uid || usuario.id || usuario.usuarioId),
        canceladoPorNome: texto(usuario.nome || usuario.nomeCompleto || usuario.email),
        motivoCancelamento: motivo,
        canceladoEm: serverTimestamp(),
      ...
      transaction.set(novoRef, novoPayload);
      transaction.set(db.collection("logs").doc(), {
        tipoAcao: "LANCAMENTO_FINANCEIRO_EDITADO",
        clientePlataformaId: tenantId,
        caixaId,
        lancamentoOriginalId,
        novoLancamentoId: novoId,
        motivo,
        usuarioId: texto(usuario.authUid || usuario.uid || usuario.id || usuario.usuarioId),
        criadoEm: serverTimestamp()
      });
      return { modo: "CRIACAO", lancamentoId: novoId, substituiLancamentoId: lancamentoOriginalId, novoSaldoCentavos };
    });
  }
  ...
  window.IntegroFinanceiroOperacional = {
    ...
    recusarSolicitacaoFinanceiraTransacional,
    cancelarSolicitacaoFinanceiraPendenteTransacional,
    cancelarLancamentoFinanceiroCaixaAbertoTransacional,
    editarLancamentoFinanceiroAdministrativoTransacional,
