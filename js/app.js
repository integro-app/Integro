...
function notificacaoPertenceAoUsuario(notificacao, usuario) {
  ...
  const idsUsuario = new Set([
    ...
    usuario.usuarioId,
    usuario.vendedorId,
    usuario.vendedorDocumentoId
  ].filter(Boolean).map(String));
  const tipo = String(usuario.tipoUsuario || usuario.cargoChave || usuario.perfil || "").toUpperCase();

  // Quando existe destinatarioAuthUid, ele é soberano: nenhum ID auxiliar
  // pode fazer outra pessoa receber a notificação.
  const destinatarioAuthUid = String(notificacao.destinatarioAuthUid || "");
  if (destinatarioAuthUid) return destinatarioAuthUid === authUid;

  const diretos = [notificacao.usuarioId, notificacao.destinatarioId].filter(Boolean).map(String);
  if (diretos.length) return diretos.some(id => idsUsuario.has(id));

  const vendedorAuthUid = String(notificacao.vendedorAuthUid || "");
  if (vendedorAuthUid) return vendedorAuthUid === authUid;

  const vendedoresLegados = [notificacao.vendedorDocumentoId, notificacao.vendedorId].filter(Boolean).map(String);
  if (vendedoresLegados.length) return vendedoresLegados.some(id => idsUsuario.has(id));

  return String(notificacao.publico || "").toUpperCase().includes(tipo) ||
