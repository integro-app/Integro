(function () {
  ...
  const STYLE_HREF = "css/integro-mobile-final.css?v=20260802-final2";
  ...
  function loadAuthoritativeStylesheet() {
    return new Promise((resolve) => {
      document.querySelectorAll(`link[href*="integro-mobile-final.css"]`).forEach((link) => link.remove());
      ...
      link.addEventListener("error", () => {
        console.error("[ÍNTEGRO] Não foi possível carregar a camada responsiva final.");
        resolve();
      }, { once: true });
      /* Inserido no fim do <body> para vencer também estilos inline legados
         existentes depois do <head>. O navegador processa normalmente folhas
         de estilo adicionadas dinamicamente neste ponto. */
