...
test("solicitação aprovada gera lançamento oficial para ingresso, gasto e retirada", async () => {
  ...
  assert.equal(db.ler("caixas/caixa_ledger").saldoAtualCentavos, 10300);
  assert.equal(db.ler("caixas/caixa_ledger").totalIngressosCentavos, 1000);
  assert.equal(db.ler("caixas/caixa_ledger").totalGastosCentavos, 300);
  assert.equal(db.ler("caixas/caixa_ledger").totalRetiradasCentavos, 400);
  assert.equal(caixaId, "caixa_ledger");
});

test("recusa financeira e atomica e nao movimenta o caixa", async () => {
  const { db, usuario } = contextoLedgerTransacional({
    "solicitacoes/recusar_sol": { clientePlataformaId: "tenant_1", caixaId: "caixa_ledger", equipeId: "equipe_1", tipo: "INGRESSO", valorCentavos: 1500, status: "PENDENTE" }
  });
  await recusarSolicitacaoFinanceiraTransacional({ usuario, solicitacaoId: "recusar_sol", clientePlataformaId: "tenant_1", motivo: "Comprovante ausente" });
  assert.equal(db.ler("solicitacoes/recusar_sol").status, "RECUSADA");
  assert.equal(db.ler("solicitacoes/recusar_sol").motivoRecusa, "Comprovante ausente");
  assert.equal(db.ler("caixas/caixa_ledger").saldoAtualCentavos, 10000);
  assert.equal(db.listar("lancamentos_financeiros").length, 0);
});
...
test("edicao e cancelamento administrativo recalculam o caixa sem apagar auditoria", async () => {
  ...
  assert.equal(db.ler("caixas/caixa_ledger").saldoAtualCentavos, 9600);
  assert.equal(db.ler("caixas/caixa_ledger").totalGastosCentavos, 400);
  await cancelarLancamentoFinanceiroCaixaAbertoTransacional({ usuario, clientePlataformaId: "tenant_1", caixaId, lancamentoId: editado.lancamentoId, motivo: "Cancelar correção", permissaoAdministrativa: true });
  assert.equal(db.ler(`lancamentos_financeiros/${editado.lancamentoId}`).statusLancamento, "CANCELADO");
  assert.equal(db.ler("caixas/caixa_ledger").saldoAtualCentavos, 10000);
  assert.equal(db.ler("caixas/caixa_ledger").totalGastosCentavos, 0);
});
