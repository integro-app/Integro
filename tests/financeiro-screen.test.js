...
const financeiroUnificado = fs.readFileSync(path.join(__dirname, "..", "js", "modules", "financeiro-unificado.js"), "utf8");
const masterLocalAtual = fs.readFileSync(path.join(__dirname, "..", "master-local.html"), "utf8");

test("financeiro administrativo unifica aprovacoes, lancamentos e caixas", () => {
  assert.match(financeiroUnificado, /\["aprovacoes","task_alt","Aprovações"\]/);
  assert.match(financeiroUnificado, /finViewAprovacoes/);
...
test("administrativo cria, edita e cancela somente pelo servico transacional", () => {
...
test("atalhos legados redirecionam ao financeiro sem gravacao direta aprovada", () => {
