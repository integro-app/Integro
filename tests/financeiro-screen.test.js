...
test("v26 separa explicitamente financeiro empresarial do operacional", () => {
  assert.match(navegacao, /__integroFinanceiroModo\s*=\s*"operacional"/);
  assert.match(navegacao, /__integroFinanceiroModo\s*=\s*"empresarial"/);
  assert.match(navegacao, /abrirFinanceiroOperacional\("lancamentos"/);
  assert.match(navegacao, /perfil\(usuarioAtual\)\s*===\s*"financeiro"/);
  assert.match(navegacao, /abrirFinanceiroEmpresarial/);
});
