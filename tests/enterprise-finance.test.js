...
test("parcelamento gera contas independentes com grupo e sequencia", () => {
  ...
  assert.match(ui, /PARCELADA/);
  assert.match(ui, /cfeFormParcelas/);
});
...
test("recorrencias suportam dia semana quinzena mes ano e geracao de ocorrencias", () => {
  ...
  assert.match(ui, /RECORRENTE/);
  assert.match(ui, /cfeRecUnidade/);
});
...
test("lembretes ficam em colecao própria e vinculados à conta", () => {
  ...
  assert.match(ui, /Lembretes/);
  assert.match(ui, /data-cfe-reminder/);
  assert.match(service, /\[7,\s*3,\s*1,\s*0\]/);
});
...
test("anexos e comprovantes usam Storage exclusivo do financeiro empresarial", () => {
  ...
  assert.match(fs.readFileSync(path.join(root, "js", "v27-bootstrap.js"), "utf8"), /firebase-storage-compat\.js/);
});
...
test("interface entrega visão geral lançamentos calendário fornecedores cadastros lembretes relatórios e histórico", () => {
  for (const label of ["Visão geral", "Lançamentos", "Calendário", "Fornecedores", "Cadastros", "Lembretes", "Relatórios", "Histórico"])
    assert.match(ui, new RegExp(label));
...
test("relatorios agregam valores sem consultar ledger operacional", () => {
  ...
  assert.match(service, /porFormaPagamento/);
  assert.match(ui, /Pago/);
});
...
test("interface declara explicitamente que caixas operacionais não entram no controle empresarial", () => {
  ...
  assert.match(ui, /independente dos caixas, vendas|não inclui movimentações de caixas|Independente dos caixas operacionais/i);
});
...
test("bootstrap v27.2 carrega o financeiro empresarial e seus guardas sem bridges operacionais", () => {
  const bootstrap = fs.readFileSync(path.join(root, "js", "v27-bootstrap.js"), "utf8");
  assert.match(bootstrap, /firebase-storage-compat-9\.22\.0/);
  assert.match(bootstrap, /v27-policy-service\.js/);
  assert.match(bootstrap, /enterprise-finance-service\.js/);
